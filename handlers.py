...
async def mes_start(message: types.Message):
    ...
    Побеждает игрок, забравший со стола последнюю конфету!\n\
...
async def mes_step(message: types.Message):
    ...
    if valid == True:    
        ...
        if total > 0:
            ...
            await message.answer(f'\
    \tНа столе осталось {total} конфет. Вы победили!!!🏆\n\
    ...
    Ваш ход. Введите "/step" и через пробел количество конфет от 1 до 28.')
            ...
            total -= bot_step
            await message.answer(f'\
    \tБот взял {bot_step}. На столе осталось {total} конфет и бот победил.✌\n\
    Если хотите поиграть еще, введите "/start".')
...
async def mes_yes(message: types.Message):
    global login
    await message.answer(f'\tУважаемый {login},\n\
